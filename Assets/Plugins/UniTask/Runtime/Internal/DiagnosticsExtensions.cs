...
namespace Cysharp.Threading.Tasks.Internal
{
	static class DiagnosticsExtensions
	{
		...
		static bool TryResolveStateMachineMethod(ref MethodBase method, out Type declaringType)
		{
			...
			MethodInfo[] methods =
				parentType.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance | BindingFlags.DeclaredOnly);
			if (methods == null)
			{
				return false;
			}
		...
		static string BeautifyType(Type t, bool shortName)
		{
			...
			if (!t.IsGenericType)
			{
				return shortName
					? t.Name
					: t.FullName.Replace("Cysharp.Threading.Tasks.Triggers.", "").Replace("Cysharp.Threading.Tasks.Internal.", "")
